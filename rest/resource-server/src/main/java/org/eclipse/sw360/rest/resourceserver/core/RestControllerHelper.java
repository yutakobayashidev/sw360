...
public class RestControllerHelper<T> {
    ...
    public VulnerabilityDTO convertToEmbeddedVulnerabilityDTOs(VulnerabilityDTO vulnerabilityDTO) {
        VulnerabilityDTO embeddedVulnerability = new VulnerabilityDTO();
        embeddedVulnerability.setExternalId(vulnerabilityDTO.getExternalId());
        return embeddedVulnerability;
    }
    public void addEmbeddedVulnerabilityDTO(HalResource halResource, List<VulnerabilityDTO> allVulnerabilityDTOs) {
        for (VulnerabilityDTO vulnerabilityDTO : allVulnerabilityDTOs) {
            VulnerabilityDTO embeddedVulnerabilityDTO = convertToEmbeddedVulnerabilityDTOs(vulnerabilityDTO);
            HalResource<VulnerabilityDTO> halAttachmentResource = new HalResource<>(embeddedVulnerabilityDTO);
            Link vulnerabilityLink = linkTo(VulnerabilityController.class)
                    .slash("api/vulnerabilities/" + embeddedVulnerabilityDTO.getExternalId()).withSelfRel();
            halAttachmentResource.add(vulnerabilityLink);
            halResource.addEmbeddedResource("sw360:vulnerabilities", halAttachmentResource);
        }
    }
