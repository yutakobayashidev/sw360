...
public class ProjectController implements RepresentationModelProcessor<RepositoryLinksResource> {
    ...
    public ResponseEntity createProjectDependency(@RequestBody Map<String, Object> reqBodyMap) throws URISyntaxException, TException {
        try {
            Project project = convertToProjectDependency(reqBodyMap, ProjectOperation.CREATE);
            if (project.getReleaseIdToUsage() != null) {
    ...
    public ResponseEntity patchProjectDependency(
        ...
        Project sw360Project = projectService.getProjectForUserById(id, user);
        try {
            Project updateProject = convertToProjectDependency(reqBodyMap, ProjectOperation.UPDATE);
            sw360Project = this.restControllerHelper.updateProject(sw360Project, updateProject, reqBodyMap, mapOfProjectFieldsToRequestBody);
    ...
    public ResponseEntity createDuplicateProjectDependency(@PathVariable("id") String id,
        ...
        try {
            updateProject = convertToProjectDependency(reqBodyMap, ProjectOperation.CREATE);
        } catch (JsonProcessingException e) {
            log.error(e.getMessage());
            return ResponseEntity.badRequest().body(e.getMessage());
        } catch (SW360Exception sw360Exception){
    ...
    private Project convertToProjectDependency(Map<String, Object> requestBody, ProjectOperation operation) throws JsonProcessingException, TException {
    ...
    public ResponseEntity createProjectDependencyReadableFormat(@RequestBody Map<String, Object> reqBodyMap) throws TException {
        ...
        User sw360User = restControllerHelper.getSw360UserFromAuthentication();
        try {
            Project project = convertFromReadableFormatToProjectDependency(reqBodyMap, ProjectOperation.CREATE, sw360User);
            project = projectService.createProject(project, sw360User);
    ...
    public ResponseEntity patchProjectDependencyReadableFormat(
        ...
        Project sw360Project = projectService.getProjectForUserById(id, user);
        try {
            Project updateProject = convertFromReadableFormatToProjectDependency(reqBodyMap, ProjectOperation.UPDATE, user);
            sw360Project = this.restControllerHelper.updateProject(sw360Project, updateProject, reqBodyMap, mapOfProjectFieldsToRequestBody);
            if(updateProject.getReleaseRelationNetwork() != null) {
                sw360Project.setReleaseRelationNetwork(updateProject.getReleaseRelationNetwork());
            }
    ...
    private Project convertFromReadableFormatToProjectDependency(Map<String, Object> requestBody, ProjectOperation operation, User sw360User) throws JsonProcessingException {
        ObjectMapper mapper = new ObjectMapper();
        mapper.configure(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, false);
        mapper.registerModule(sw360Module);
