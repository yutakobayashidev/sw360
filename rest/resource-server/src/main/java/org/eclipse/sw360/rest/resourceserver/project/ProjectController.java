...
public class ProjectController implements RepresentationModelProcessor<RepositoryLinksResource> {
    ...
    public ResponseEntity createProjectDependency(@RequestBody Map<String, Object> reqBodyMap) throws URISyntaxException, TException {
        try {
            Project project = convertToProjectDependency(reqBodyMap);
            if (project.getReleaseIdToUsage() != null) {
    ...
    public ResponseEntity patchProjectDependency(
        ...
        Project sw360Project = projectService.getProjectForUserById(id, user);
        try {
            Project updateProject = convertToProjectDependency(reqBodyMap);
            sw360Project = this.restControllerHelper.updateProject(sw360Project, updateProject, reqBodyMap, mapOfProjectFieldsToRequestBody);
    ...
    public ResponseEntity<CollectionModel<EntityModel<Release>>> getProjectReleaseDependency(
        ...
        final List<EntityModel<Release>> releaseResources = paginationResult.getResources().stream()
                .map(sw360Release -> wrapTException(() -> {
                    ...
                    final HalResource<Release> releaseResource = new HalResource<>(embeddedRelease);
                    if (isTransitive) {
                        projectService.addEmbeddedReleaseInNetwork(sw360Release, sw360User, releaseResource,releaseService);
                    }
                    return releaseResource;
                })).collect(Collectors.toList());
    ...
    public ResponseEntity<CollectionModel<EntityModel<Release>>> getProjectsReleasesNetwork(
            Pageable pageable,
            @RequestBody List<String> projectIds,
            @RequestParam(value = "clearingState", required = false) String clState,
            @RequestParam(value = "transitive", required = false) String transitive, HttpServletRequest request) throws URISyntaxException, PaginationParameterException, ResourceClassNotFoundException {

        final User sw360User = restControllerHelper.getSw360UserFromAuthentication();
        boolean isTransitive = Boolean.parseBoolean(transitive);
        List<ReleaseLinkJSON> releasesLinkedDirectly = projectService.getReleasesInDependencyFromProjectIds(projectIds, sw360User, isTransitive);
    ...
    @RequestMapping(value = PROJECTS_URL + "/dependency/{id}/licenseinfo", method = RequestMethod.GET)
    public void downloadLicenseInfoFromNetwork(@PathVariable("id") String id,
                                    @RequestParam("generatorClassName") String generatorClassName,
                                    @RequestParam("variant") String variant,
                                    @RequestParam(value = "externalIds", required=false) String externalIds,
                                    @RequestParam(value = "template", required = false ) String template,
        ...
        List<ProjectLink> mappedProjectLinks = projectService.createLinkedProjectsForNetwork(sw360Project,
                projectService.filterAndSortAttachments(SW360Constants.LICENSE_INFO_ATTACHMENT_TYPES), true, sw360User);
    ...
    private void getSubReleaseIds(List<ReleaseLinkJSON> releaseLinks, Set<String> releaseIds) {
    ...
    @PreAuthorize("hasAuthority('WRITE')")
    @RequestMapping(value = PROJECTS_URL + "/dependency/readableFormat", method = RequestMethod.POST)
    public ResponseEntity createProjectDependencyReadableFormat(@RequestBody Map<String, Object> reqBodyMap) throws TException {
        ComponentService.Iface componentService = new ThriftClients().makeComponentClient();
        User sw360User = restControllerHelper.getSw360UserFromAuthentication();
        try {
            Project project = convertFromReadableFormatToProjectDependency(reqBodyMap, sw360User);
            project = projectService.createProject(project, sw360User);
    ...
    @PreAuthorize("hasAuthority('WRITE')")
    @RequestMapping(value = PROJECTS_URL + "/dependency/readableFormat/{id}", method = RequestMethod.PATCH)
    public ResponseEntity patchProjectDependencyReadableFormat(
            @PathVariable("id") String id,
            @RequestBody Map<String, Object> reqBodyMap) throws TException {
        User user = restControllerHelper.getSw360UserFromAuthentication();
        Project sw360Project = projectService.getProjectForUserById(id, user);
        try {
            Project updateProject = convertFromReadableFormatToProjectDependency(reqBodyMap, user);
            sw360Project = this.restControllerHelper.updateProject(sw360Project, updateProject, reqBodyMap, mapOfProjectFieldsToRequestBody);
            if(updateProject.getReleaseRelationNetwork() != null) {
                sw360Project.setReleaseRelationNetwork(updateProject.getReleaseRelationNetwork());
            }
    ...
    private Project convertFromReadableFormatToProjectDependency(Map<String, Object> requestBody, User sw360User) throws JsonProcessingException, TException {
        ComponentService.Iface componentService = new ThriftClients().makeComponentClient();
        ...
        for (ReleaseLinkJSON release : inputNetwork) {
            if (!mapIndexOfSubRelease.containsKey(release.getReleaseId())) {
                ...
                componentService.getReleaseById(release.getReleaseId(), sw360User);
                String mainLineStateUpper = (release.getMainlineState() != null) ? release.getMainlineState().toUpperCase() : MainlineState.OPEN.toString();
                String releaseRelationShipUpper = (release.getReleaseRelationship() != null) ? release.getReleaseRelationship().toUpperCase() : ReleaseRelationship.CONTAINED.toString();

                if (!enumMainlineStateValues.contains(mainLineStateUpper)) {
                    throw new NoSuchElementException("mainLineState of release " + release.getReleaseId() + " must be in Enum " + enumMainlineStateValues);
                }
                if (!enumReleaseRelationshipValues.contains(releaseRelationShipUpper)) {
                    throw new NoSuchElementException("releaseRelationShip of release " + release.getReleaseId() + " must be in Enum " + enumReleaseRelationshipValues);
                }

                releaseLinkJSON.setReleaseId(release.getReleaseId());
                releaseLinkJSON.setMainlineState(mainLineStateUpper);
                releaseLinkJSON.setReleaseRelationship(releaseRelationShipUpper);
                ...
                if (release.getReleaseLink() == null || release.getReleaseLink().size() == 0) {
                    ...
                    releaseLinkJSON.setReleaseLink(getRelationNetwork(mapIndexOfSubRelease, release, inputNetwork, sw360User));
                }
    ...
    private List<ReleaseLinkJSON> getRelationNetwork(Map<String, Integer> listSubReleaseId, ReleaseLinkJSON releaseLinkJSON, List<ReleaseLinkJSON> inputNetwork, User sw360User) {
        ...
        for (ReleaseLinkJSON subRelease : releaseLinkJSON.getReleaseLink()) {
            ...
            componentService.getReleaseById(releaseByIndex.getReleaseId(), sw360User);
            String mainLineStateUpper = (releaseByIndex.getMainlineState() != null) ? releaseByIndex.getMainlineState().toUpperCase() : MainlineState.OPEN.toString();
            String releaseRelationShipUpper = (releaseByIndex.getReleaseRelationship() != null) ? releaseByIndex.getReleaseRelationship().toUpperCase() : ReleaseRelationship.CONTAINED.toString();

            if (!enumMainlineStateValues.contains(mainLineStateUpper)) {
                throw new NoSuchElementException("mainLineState of release " + subRelease.getReleaseId() + " must be in Enum " + enumMainlineStateValues);
            }
            if (!enumReleaseRelationshipValues.contains(releaseRelationShipUpper)) {
                throw new NoSuchElementException("releaseRelationShip of release " + subRelease.getReleaseId() + " must be in Enum " + enumReleaseRelationshipValues);
            }

            release.setReleaseId(releaseByIndex.getReleaseId());
            ...
            release.setReleaseLink(getRelationNetwork(listSubReleaseId, releaseByIndex, inputNetwork, sw360User));
